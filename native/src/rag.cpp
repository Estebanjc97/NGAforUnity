...
struct SearchOptsGuard 
{ 
    ACESearchOptions* p = nullptr;
    ~SearchOptsGuard() 
    { 
        if (p) ace_destroySearchOptions(p); 
    } 
};

struct SearchResultsGuard 
{ 
    ACESearchResults* p = nullptr;
    ~SearchResultsGuard() 
    { 
        if (p) ace_destroySearchResults(p); 
    } 
};

struct DbParamsGuard 
{
    ACEDatabaseLoadParams* p = nullptr;
    ~DbParamsGuard() 
    { 
        if (p) ace_destroyDatabaseLoadParams(p); 
    } 
};
...
{
    NGA_TRY_BEGIN
        if (!runtime_handle || !runtime_handle->ctx) 
        {
        ...
        *out = nullptr;
        ...
        if (!kbConfigJson || !kbConfigJson[0]) 
        { 
            nga_set_last_error("kbConfigJson null/empty");
            return NGA_ERR_INVALID_ARG;
        }
        ...
        try 
        { 
            cfg = json::parse(kbConfigJson); 
        }
        catch (const std::exception& e) 
        { 
            nga_set_last_error(std::string("kb JSON error: ") + e.what());
        ...
        if (semPath.empty() && lexPath.empty()) 
        { 
            nga_set_last_error("specifies semanticDbPath and/or lexicalDbPath"); 
            return NGA_ERR_INVALID_ARG;
        }
...
{
    NGA_TRY_BEGIN
        if (!kb || !kb->rt || !kb->rt->ctx)
        { 
            nga_set_last_error("kb null/no inicializado");
            return NGA_ERR_INVALID_ARG;
        }
        if (!query || !query[0]) 
        {
            nga_set_last_error("query null/vacio");
            return NGA_ERR_INVALID_ARG;
        }

        SearchOptsGuard opts;
        ACEResult r = ace_createSearchOptions(kb->rt->ctx, &opts.p);

        if (r != ACEResultOk) 
            return nga_translate_result(r, "ace_createSearchOptions");
        if (topK > 0) 
            ace_setSearchOptionInt(opts.p, ACESearchOption_MaxNumResults, topK);

        SearchResultsGuard results;
        if (mode == NGA_RAG_SEMANTIC)
        {
            if (!kb->semantic) 
            {
                nga_set_last_error("the KB does not have semantic index");
                return NGA_ERR_NOT_FOUND;
            }
            r = ace_databaseSearch(kb->semantic, query, opts.p, &results.p);
            if (r != ACEResultOk) 
                return nga_translate_result(r, "ace_databaseSearch(semantic)");
        }
        else if (mode == NGA_RAG_LEXICAL)
        {
            if (!kb->lexical)
            {
                nga_set_last_error("KB does not have lexical index");
                return NGA_ERR_NOT_FOUND; 
            }
            r = ace_databaseSearch(kb->lexical, query, opts.p, &results.p);
            if (r != ACEResultOk) 
                return nga_translate_result(r, "ace_databaseSearch(lexical)");
        }
        else
        { // NGA_RAG_HYBRID
            if (!kb->semantic || !kb->lexical)
            {
                nga_set_last_error("hybrid requires semantic and lexical indexes");
                return NGA_ERR_NOT_FOUND;
            }
            ...
            if (r != ACEResultOk) 
                return nga_translate_result(r, "ace_hybridSearch");
        }
